...
- **If the feature's screen(s) already exist live in FusionX UAT and the flow is stable** (not
  ...
  details in what's actually there. Check for browser automation access before assuming it's
  unavailable — a `playwright-cli` command (or `npx playwright cli`) may be reachable via Bash even
  when no dedicated browser MCP tool is registered; confirm with `playwright-cli --version` (or the
  npx form) rather than concluding "no Playwright access" from tool-search results alone. Keep the
  browser visible for the user to log in/complete MFA and wait for their confirmation before
  touching authenticated screens. Once in, follow the user's own Playwright Full-Coverage standing
  instructions while walking the flow (full-depth by default not a breadth-only click-through,
